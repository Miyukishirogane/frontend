...
import { dataTableLiquidityHeader, displayBgTableCell, displayRankTableCell } from 'src/views/LeaderBoard/util';
...
export const TableLiquidityRowItem = ({ data }: { data: LeaderboardTableType }) => {
  return (
    <>
      <TableCell>
        <TextSmallNumber value={BN(data.totalLiquidityVolume)} decimal={2} fallbackDisplay="--" />
      </TableCell>

      <TableCell>
        <TextSmallNumber value={BN(data.scoreLiquidityPosition)} decimal={2} fallbackDisplay="--" />
      </TableCell>

      <TableCell>
        <TextSmallNumber value={BN(data.scoreLiquidityCamelot)} decimal={2} fallbackDisplay="--" />
      </TableCell>
    </>
  );
};

const TableLiquidity = (props: IProps) => {
  ...
  return (
    <Table stickyHeader>
      <TableHead>
        <TableRow>
          {dataTableLiquidityHeader.map(item => (
            <TableCell
      ...
      <TableBody>
        {loading ? (
          <SkeletonTableBody cols={dataTableLiquidityHeader.length} />
        ) : (
          dataTable?.map((item, index) => {
            const rank = item.rankLiquidityPosition || index;
            ...
            return (
              <TableRow sx={{ background: displayBgTableCell(rank) }} key={item?.id || index}>
                ...
                <TableLiquidityRowItem data={item} />
              </TableRow>
            );
          })
        )}

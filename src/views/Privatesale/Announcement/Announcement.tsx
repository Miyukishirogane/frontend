...
import Star3Blink from 'src/views/Liquidity/Announcement/Star/Star3Blink';
import { useUserPrivateSale } from 'src/jotai/userSale/userPrivateSale';

export default function Announcement() {

    // jotai data
    const { dataList, loadingPage } = useUserPrivateSale()
    ...
    return (
        ...
        >
            ...
            <Box sx={{ width: '100%', ml: { md: 10, xs: 4 }, pb: { md: 7, xs: 4 }, }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', mt: { md: 1, xs: 1 } }} style={{ cursor: 'pointer' }}>
                    {loadingPage ? (
                        ...
                        <Countdown targetTimestamp={parseInt(dataList[0]?.terms?.vestingTimeStart) || 0} />
                    )}
                </Box>
            </Box>
            <BannerImage sx={{ right: '17%', top: '10%', width: '6%', zIndex: '100' }} image={imagePath.KEY} />
            <BannerImage sx={{ right: '24%', top: { lg: '20%', xsm: '20%', sm: '28%', xs: '30%' }, width: '10%', zIndex: '100' }} image={imagePath.COIN} check={false} />
            <BannerImage sx={{ right: '30%', top: '60%', width: '8%', zIndex: '100' }} image={imagePath.PERCENT} />
            <BannerImage sx={{ right: '7%', top: { lg: '28%', xsm: '28%', sm: '33%', xs: '46%' }, width: '15%', zIndex: '100' }} image={imagePath.COINBIG} check={true} />
            ...
            <Star3Blink sx={{ right: '5%', top: '70%', width: '25px', rotate: '10deg', animationDelay: '5000ms' }} />
        </Box >
    );
}
    ...
    return (
        <>
            {timeRemaining.hours > 0 || timeRemaining.minutes > 0 || timeRemaining.seconds > 0 ? (
                <>
                    <Box sx={{ display: 'flex', alignItems: 'center', mt: { md: 1, xs: 1 } }} style={{ cursor: 'pointer' }}>
                        <Box color={'#fff'} sx={{ fontSize: { xs: '14px', xsm: '20px', sm: '24px', lg: '40px' }, padding: '10px', fontWeight: '600', background: 'rgba(255, 255, 255, 0.20)', borderRadius: '10px' }}>

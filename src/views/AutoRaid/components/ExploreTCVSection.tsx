...
const ExploreTCVSection = () => {
  const { tcvCategory, status } = useGetUserTask();

  const totalTaskCompleted = tcvCategory?.filter(item => item.done).length || 0;
  const totalPoint = tcvCategory?.reduce((acc, item) => acc + item.point, 0) || 0;
    ...
    >
      <Box display="flex" mb={2} justifyContent="space-between" alignItems="center">
        ...
        <TypographyByStatus status={status} variant="body1" fontWeight={600}>
          {totalTaskCompleted}/{tcvCategory?.length}
        </TypographyByStatus>
      </Box>
      ...
      <Box display="flex" flexDirection="column" gap={1}>
        {status === 'pending' ? (
          ...
          tcvCategory?.map(task => <RaidTaskItem key={task.id} task={task} />)
        )}
      </Box>
    </Paper>
  );
...
export default ExploreTCVSection;

...
export default function useGetDelegated() {
  const chainId = useChainId();
  const { address } = useAccount();

  const delegated = useQuery({
    queryKey: ['useGetDelegated', address],
    queryFn: async () => {
      if (address) {
        const delegated = await readContract(configEvmChain, {
          abi: abiVotingEscrow,
  ...
  return { ...delegated, delegated: delegated.data };
}
